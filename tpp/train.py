...
LR_CONFIG = {
	"max_lr": 3e-3,
	"warmup_steps": int(0.1 * TRAIN_CONFIG["max_steps"]),
...
def main():
	...
	if args.pretrained is not None:
		model = TPPForCausalLM.from_pretrained(args.pretrained)
	else:
		config = TPPConfig(
			vocab_size=50304,
			n_embed=768,
	...
	for step in prog_bar:
		...
		for micro_step in range(grad_accum_steps):
			...
			# TODO: check if autocast to bf16 causes issues on unsupported hw
